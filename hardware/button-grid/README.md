...
- **Routing**: Column traces run on F.Cu (vertical), row traces on B.Cu
  (horizontal). Vias at each diode connect front switch pad to back row trace.
- **LED power**: VCC and GND traces need to handle ~7.7A peak (128 LEDs ×
  60mA). Use wide traces (≥1mm) or copper pours. The generated board has a
  GND fill on B.Cu and a VCC fill on F.Cu in the connector area.
- **LED data**: Keep the data line short between consecutive LEDs. The snaking
  layout ensures adjacent chain LEDs are physically adjacent.
